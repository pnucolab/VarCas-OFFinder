<script>
   ...
   async function get_result(ticket){
	...
	const{status, created_at, finished_at} = result;
	if (result.status === 1) {
...
$: filteredData = classifiedData.filter(item =>
	item.GC.trim().startsWith(searchTerm.trim()) ||
...
{#if result.status == 0}
 ...
 <input class="search-input" type="text" placeholder="Search by mismatches or GC content..." bind:value={searchTerm}>
