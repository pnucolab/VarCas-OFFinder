...
async def cas_offinder(Target_Genome: Target_Genome, Pam: Pam, 
        ...
        ticket = task_id
        rofftarget=off_target.apply_async([ticket, output_file_name, file_name, output_vcf, ref_path, pam_line, target_line ], task_id=task_id)
...
async def result(ticket:str):
   ...
   if ticket in task_id_lists:
        ...
        if status == "PENDING":
            ...
            specific_id = ticket
            query = "SELECT created_at, finished_at FROM task_times WHERE id = ?"
